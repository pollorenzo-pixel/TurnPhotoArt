...
export function PrivateAccessForm() {
  ...
  return <form className="access-form" onSubmit={submit} aria-busy={busy}><label htmlFor="access-code">Private access code <span>Required</span></label><input id="access-code" type="password" autoComplete="one-time-code" value={code} onChange={(e)=>setCode(e.target.value)} maxLength={200} required aria-invalid={Boolean(error)} aria-describedby={error?"access-error":undefined}/><button className="button button-primary" type="submit" disabled={busy}>{busy?"Checking access…":"Enter private test"}</button>{error?<p id="access-error" role="alert">{error}</p>:null}</form>;
}
